...
print('CALCULATOR ENTRIES START')
for entry in state_pages:
    ...
    print(f"    name: '{name}',")
    print(f"    title: '{title}',")
    print(f"    metaDescription: '{meta}',")
    print(f"    h1: '{name}',")
    print(f"    h2: '{name} Tool',")
    print(f"    description: 'The {name} provides state-specific GPA planning resources and a fast calculator for students in {state}.',")
    print(f"    targetAudience: '{state} Students',")
    print('    relatedCalculators: [\'college-gpa-calculator\', \'cumulative-gpa-calculator\', \'semester-gpa-calculator\', \'weighted-gpa-calculator\', \'unweighted-gpa-calculator\'],')
    print('  },')
print('CALCULATOR ENTRIES END')
print('TEMPLATE ENTRIES START')
for entry in state_pages:
    id_, name, state, url = entry
    short = 'Texas College' if id_ == 'texas-college-gpa-calculator' else state
    print('  {')
    print(f"    id: '{id_}',")
    print(f"    schoolName: '{name}',")
    print(f"    shortName: '{short}',")
    print(f"    location: '{state}',")
    print(f"    strengths: 'strong regional academic support, state university systems, and transfer-friendly GPA policies in {state}',")
    print('    externalLinks: [')
    print(f"      {{ label: '{state} Higher Education Authority', url: '{url}' }},")
    print("      { label: 'National Center for Education Statistics', url: 'https://nces.ed.gov/' },")
    print("      { label: 'College Board', url: 'https://www.collegeboard.org/' },")
    print('    ],')
    print('  },')
print('TEMPLATE ENTRIES END')
