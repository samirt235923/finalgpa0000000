...
const harvardGradePoints: Record<string, number> = {
  'A': 4.0,
  'A-': 3.7,
  'B+': 3.3,
  'B': 3.0,
...
export default function HarvardGPACalculator() {
  ...
  const results = useMemo(() => {
    ...
    courses.forEach((course) => {
      const gradeValue = harvardGradePoints[course.grade] ?? 0;
  ...
  return (
    <div className="bg-white border-2 border-gray-200 rounded-2xl shadow-sm p-4 md:p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h2 className="text-xl font-bold text-gray-900">Harvard GPA Calculator</h2>
      ...
      <div className="mb-4 p-3 bg-red-50 rounded-lg text-sm text-gray-700">
        Enter your courses and grades using Harvard's grading scale. The calculator uses standard Harvard grade points (A=4.0, A-=3.7, etc.)
      </div>
              ...
              <tr key={course.id} className="border-t border-gray-200">
                ...
                <td className="px-2 py-2 w-36">
                  <select
                    ...
                    className="w-full border border-gray-300 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    {Object.entries(harvardGradePoints).map(([grade, points]) => (
      ...
      {results && (
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-lg p-4 bg-blue-50 border-2 border-blue-200">
            <p className="text-xs text-blue-600 uppercase font-semibold">Harvard GPA</p>
      ...
      <div className="mt-4 p-4 bg-gray-100 rounded-xl">
        <h3 className="font-semibold text-gray-800 mb-2">Harvard Grade Scale</h3>
